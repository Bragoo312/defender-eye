...
### Симптом
В Open Defender версии до v1.3.1 IP-адрес атакующего в событиях eBPF отображается развернутым задом наперёд, а порт `23` считывается как `5888`.

### Решение
1. Откройте **Defender Eye → Настройки**.
2. В секции **Авто-патчер eBPF модуля Open Defender** нажмите **«Проверить патч»**.
3. Панель определит версию установленного Open Defender (`v1.3.1+` или бинарник/исходник).
4. Нажмите **«Применить авто-патч»**. Панель сделает бэкап `.bak`, заменит порядок байт и выполнит `systemctl restart open-defender`.

---

## 6. Дисковый накопитель отображает 0 B / 0 B

### Симптом
Во вкладке Системная телеметрия диск показывает `0 B / 0 B`.

### Решение
В Defender Eye v1.3.0 добавлена реализация вызова `syscall.Statfs("/", &stat)` для раздела Linux `/`. Обновите панель до версии v1.3.0.
